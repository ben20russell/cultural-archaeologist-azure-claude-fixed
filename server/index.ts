...
app.get('/api/validate-article', async (req, res) => {
  const rawUrl = Array.isArray(req.query.url) ? req.query.url[0] : req.query.url;
  ...
  if (parsedUrl.protocol !== 'http:' && parsedUrl.protocol !== 'https:') {
    return res.status(400).json({ error: 'Only http/https article URLs are allowed.' });
  }
  ...
  try {
    const response = await fetch(parsedUrl.toString(), {
      ...
      headers: {
        ...
        'User-Agent': 'BrandAtlasArticleValidator/1.0',
      },
    });
    ...
    if (!response.ok || !isHtml) {
      console.log('[article-validator] URL failed status/content-type checks.', {
    ...
    const hasContent = textLength >= 250;

    const titleMatch = html.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
    const publishedMetaMatch =
    ...
    const publishedAt = publishedMetaMatch?.[1]?.trim() || null;

    console.log('[article-validator] URL validated.', {
      url: parsedUrl.toString(),
      finalUrl,
      status,
      hasContent,
    ...
    console.log('[article-validator] URL validation failed.', { url: parsedUrl.toString(), message });
    return res.status(200).json({
      isReachable: false,
      hasContent: false,
      error: message,
